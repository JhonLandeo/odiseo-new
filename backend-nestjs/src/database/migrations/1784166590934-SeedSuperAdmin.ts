...
export class SeedSuperAdmin1784166590934 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        // 7. Assign role
        await queryRunner.query(`INSERT INTO "${sysSchema}".user_roles (user_id, role_id) VALUES ($1, $2)`, [sysUserId, sysRoleId]);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        // En un rollback, idealmente borraríamos el esquema tenant del super admin y el plan
        const sysCompany = await queryRunner.query(`SELECT id FROM public.companies WHERE subdomain = 'odiseo'`);
        if (sysCompany.length > 0) {
            ...
            await queryRunner.query(`DELETE FROM public.companies WHERE id = $1`, [sysCompany[0].id]);
        }
        await queryRunner.query(`DELETE FROM public.subscription_plans WHERE name = 'Free'`);
    }
