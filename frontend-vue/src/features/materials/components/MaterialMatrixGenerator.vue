            ...
            <div class="flex-1 flex flex-col lg:flex-row min-h-0">
              ...
              <div class="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                ...
                <div v-else-if="selectedCycleId && academicStore.isLoading && templates.length === 0" class="space-y-6">
                  <div v-for="n in 2" :key="n"
                    ...
                    <div
                      class="px-5 py-4 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-black/10">
                ...
                <div v-else-if="templates.length === 0"
                  class="py-16 px-6 text-center max-w-md mx-auto flex flex-col items-center">
                  <div
                    class="w-16 h-16 rounded-full bg-amber-50 dark:bg-amber-950/35 flex items-center justify-center text-amber-500 mb-4 ring-8 ring-amber-500/10 animate-pulse">
                    <UIcon name="i-heroicons-exclamation-triangle" class="w-8 h-8" />
                  </div>
                  <h3 class="text-lg font-black text-slate-800 dark:text-slate-200 tracking-tight">Sin Plantillas
                    Configuradas</h3>
                  <p class="text-xs text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">
                    Para poder generar materiales y continuar con el flujo, es obligatorio que configures al menos una
                    plantilla de
                  ...
                  <div v-for="template in templates" :key="template.id"
                    ...
                    <div
                      ...
                      <div>
                        ...
                        <p class="text-xs text-slate-500 mt-0.5 ml-6">{{ template.courses.length }} cursos participan
                        </p>
                      </div>
                    </div>
                      ...
                      <div class="grid grid-cols-5 sm:grid-cols-8 lg:grid-cols-10 gap-2">
                        <button v-for="week in weeksList" :key="week" @click="selectIntersection(template.id, week)"
                          ...
                          <span v-if="weekStatusMap[`${template.id}_${week}`]"
                            class="absolute -top-1 -right-1 w-3 h-3 rounded-full ring-2 ring-white dark:ring-[#1a1a24] z-20"
                            :class="{
              ...
              <Transition enter-active-class="transition ease-out duration-300"
                ...
                <div v-if="selectedTemplate && selectedWeek"
                  ...
                  <Transition mode="out-in" enter-active-class="transition-opacity duration-150"
                    enter-from-class="opacity-0" enter-to-class="opacity-100"
                    leave-active-class="transition-opacity duration-100" leave-from-class="opacity-100"
                    leave-to-class="opacity-0">
                    <div :key="`${selectedTemplate?.id}_${selectedWeek}`" class="p-5 space-y-5">
                      ...
                      <div class="border-t border-slate-200 dark:border-white/5 pt-4">
                        <PdfDesignSelector v-model="selectedDesignId"
                          ...
                          :week-number="selectedWeek || 1" :template-name="selectedTemplate?.name || 'Material'"
                      ...
                      <Transition enter-active-class="transition ease-out duration-200"
                        ...
                        <div v-if="showCoursesList && currentRequest"
                          class="border-t border-slate-200 dark:border-white/5 pt-4 space-y-3">
                          <h4
                          ...
                          <div class="space-y-2">
                            <div v-for="course in currentRequest.courses" :key="course.courseId + '_' + course.status"
                              ...
                              <div class="flex items-center gap-2 shrink-0">
                                ...
                                <UButton size="xs" color="neutral" variant="ghost" icon="i-heroicons-list-bullet"
                      ...
                      <div class="border-t border-slate-200 dark:border-white/5 pt-4 space-y-3">
                        <h4
                          class="text-[10px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-1.5">
                        ...
                        <div v-else-if="attemptsHistory.length === 0"
                        ...
                        <div v-else class="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                          <div v-for="attempt in attemptsHistory" :key="attempt.id"
                            ...
                            <div class="flex items-center gap-2">
                              <span
                                class="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border shrink-0"
                                :class="{
        ...
        <div v-if="isReviewModalOpen"
          ...
          <div
            ...
            <div class="overflow-y-auto custom-scrollbar flex-1 px-6 pb-6">
              <MaterialReviewList v-if="isReviewModalOpen && currentRequest"
                :key="currentRequest.id + '_' + (activeReviewCourseId || 'all')" :materialId="currentRequest.id"
                :initialCourseId="activeReviewCourseId || undefined" @approved="handleReviewApproved"
